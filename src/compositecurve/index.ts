...
console.log("Hello World from compositecurve");
type Identifier = {
    index: number;
    type: "CP" | "LH" | "RH";
}
...
// console.log(element.zoomHandler);
// element.zoomHandler.restrictZoom = true;
element.controlCenter.zoomController.restrictZoom = true;
...
if(dragButton){
    dragButton.onclick = ()=>{
        ...
        if(dragControlPoint){
            element.panHandler.restrictTranslation = true;
        } else {
            element.panHandler.restrictTranslation = false;
        }
    }
}
...
function step(timestamp: number){
    element.step(timestamp);
    testCurve.draw(context, element.camera.zoomLevel);
